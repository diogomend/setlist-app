...
describe("appState getters", () => {
  ...
  it("expects getArtists() to return list of artists", () => {
    let state = {
      setlists: [{artist: {name: "A", mbid: 123}}, {artist: {name: "B", mbid: 321}}, {artist: {name: "B", mbid: 321}}]
    };

    expect(setlist.getters.getArtists(state)).toEqual([{"mbid": 123, "qty": 1, "name": "A"}, {"mbid": 321, "qty": 2, "name": "B"}]);
  });
  ...
  it("expects getArtistShows() to return shows of artist", () => {
    ...
    expect(setlist.getters.getArtistShows(state)(321)).toEqual([{artist: {"mbid": 321, "name": "B"}}, {artist: {"mbid": 321, "name": "B"}}]);
  });
  ...
  it("expects getTopArtists() to return top artists", () => {
    ...
    expect(setlist.getters.getTopArtists(state)).toEqual([ { qty: 5 }, { qty: 1 } ]);
  });

  it("expects getYearlyConcerts() to return concerts by year", () => {
    const state = {
      setlists: [{eventDate: "06-06-2016"}, {eventDate: "06-06-2018"}, {eventDate: "06-10-2018"}]
    };
